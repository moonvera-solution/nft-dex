...
import {MvxMarket} from "@src/MvxMarket.sol";
...
contract MarketTest is Test {
    ...
    function _generateMarketItem(uint256 _tokenId, address _quoteAsset)
        internal
        returns (MvxMarket.MarketItem memory item)
    {
        item = MvxMarket.MarketItem({
            itemId: 0, // uint
    ...
    function _listItem(MvxMarket.MarketItem memory item, uint256 _tokenId) public payable {
    ...
    function test_listItem(uint256 _tokenId) public {
        MvxMarket.MarketItem memory item = _generateMarketItem(_tokenId, address(0x0));
    ...
    function test_buyItem_with_ERC20(uint256 _tokenId) public {
        // list new item
        MvxMarket.MarketItem memory item = _generateMarketItem(_tokenId, address(usdc));
        _listItem(item, _tokenId);
